...
def init():
    global pits, current_position, goal
    pits = [(1, 1), (2, 2), (3, 3)]
    current_position = (0, 0)
    goal = (4, 4)
...
@app.get("/get_data")
async def get_data():
    return {"pits": pits, "current_position": current_position, "goal": goal}

@app.get("/step")
async def step():
    global current_position, done
    if done:
        return {"x": current_position[0], "y": current_position[1], "reward": 0, "done": True}
    
    # move agent and get feedback
    ...
    # calc and set new q value
    new_q_value = old_q_value + alpha * (step_reward + gamma * max_future_q - old_q_value)
    q_table[old_state][action] = new_q_value
    
    return {"x": current_position[0], "y": current_position[1], "reward": step_reward, "done": done}

@app.post("/reset_data")
async def reset_data():
    global pits, current_position, goal, done, epsilon, epsilon_decay
    pits = [(1, 1), (2, 2), (3, 3)]
    current_position = (0, 0)
    goal = (4, 4)
